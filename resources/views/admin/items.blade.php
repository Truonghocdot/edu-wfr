...
<body>
    <div class="dashboard">
        ...
        <main class="main-content">
            ...
            <div class="actions-bar" style="padding: 0 20px 20px; display: flex; gap: 10px;">
                <form action="{{ route('items') }}" method="GET" style="display: flex; gap: 10px; width: 100%;">
                    ...
                    <button type="submit" class="btn"
                        style="background: #e67e22; color: white; border: none; padding: 0 20px; border-radius: 6px; cursor: pointer;">Search</button>
                </form>
            </div>
                        ...
                        <tbody>
                            @forelse($items as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->category }}</td>
                                    <td><span
                                            class="badge {{ $item->type === 'lost' ? 'badge-danger' : 'badge-success' }}">{{ ucfirst($item->type) }}</span>
                                    </td>
                                    <td>{{ $item->location }}</td>
                                    <td>{{ $item->date_reported->format('d/m/Y') }}</td>
                                    <td><span class="status {{ $item->status }}">{{ ucfirst($item->status) }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ $item->type === 'found' ? route('viewFoundItem', $item->id) : route('viewLostItem', $item->id) }}"
                                        ...
                                        <form action="{{ route('adminDeleteItem', $item->id) }}" method="POST"
